...
select MAX(total_qty) as total_qty,storeid, storename from (
	select SUM(transaction.qty)as total_qty, transaction.storeid as storeid, store.storename as storename from transaction join store on store.storeid = transaction.storeid group by store.storename, transaction.storeid
...
GROUP BY product.productid, product."Product Name" order by amount desc;